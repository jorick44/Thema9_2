...
Let's use statistical tests to test the skewness.
...
```{r boxplot transformed data, fig.height=10, fig.width=20, message=FALSE, warning=FALSE}
...
```{r qqlpots, fig.height=10, fig.width=20, message=FALSE, warning=FALSE}
...
The data is despite the transformation still not fully normalised however we can still continue but keep this in mind for future.

```{r new dataframe}
new_data <- cbind(data[3:6], trans[2:6])
```

Using the new data let's explore if the data is correlated.
...
REG1 A and B seem moderately correlated otherwise no real correlation is observed.
